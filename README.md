...
在 winXray 中管理 PAC 名单非常的简洁方便，如果一个代理客户端 PAC 模式卡顿不稳定，编辑 PAC 麻烦困难，或者根本就不支持 PAC，这时候他们会说是因为 PAC 不好，这有多少可信度我就不多说了。请注意**只有 PAC 能真正的直接支持高效安全的 SOCKS5 协议**，否则就只能改为低版本 SOCKS 或者 HTTP 代理。**PAC 与翻墙软件完全无关并完全独立**，可以保证只有你指定的国外网站与翻墙软件发生关系。**PAC 能避免一些敏感的软件不会在没经过你手动许可时悄悄流向翻墙软件**，例如迅雷或 Telegram。另外启用 PAC 并不会影响大家使用 Xray-Core 自带的路由规则 - 这与 PAC 并没有任何的矛盾和冲突。  
